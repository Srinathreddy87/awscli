...
def validate_data(self, before_table, after_table):
    ...
    # Prepare data for the audit table
    results = self.prepare_results(comparison_df, df_a.columns, before_table, after_table)
...
def create_join_condition(self, df_a, df_b):
    ...
    Create join condition based on all columns.
    """
    return [df_a[f"{col}_a"] == df_b[f"{col}_b"] for col in df_a.columns if col.endswith("_a")]
...
def construct_comparison_query(self, df_a):
    ...
    SELECT
        ...
        {', '.join(comparison_columns)},
        CASE
            WHEN {' OR '.join([f'{col.replace("_a", "")}_result = \'unmatch\'' for col in df_a.columns if col.endswith("_a")])}
            THEN 'unmatch'
            ELSE 'match'
        END AS validation_result
    FROM joined_view
...
def prepare_results(self, comparison_df, columns, before_table, after_table):
    """
    Prepare data for the audit table.
    """
    results = []
    ...
    for col in columns:
        if col.endswith("_a"):
            mismatch_count = comparison_df.filter(f"{col.replace('_a', '')}_result = 'unmatch'").count()
            results.append({
                "test_name": "ABTest",
                "table_a": before_table,
                "table_b": after_table,
                "column_name": col.replace('_a', ''),
                "schema_mismatch": col.replace('_a', '') not in columns,
                "data_mismatch": mismatch_count > 0,
                "mismatch_count": mismatch_count,
...
def write_results(self, results_df, audit_table_name):
    ...
    """
    try:
        results_df.write.format("delta").mode("append").saveAsTable(audit_table_name)
        logger.info("Data validation complete. Comparison results stored in table: %s", audit_table_name)
    except Exception as e:
        logger.error("Failed to save comparison results: %s", e)
...
def write_comparison_results(self, comparison_df):
    ...
    """
    try:
        comparison_df.write.format("delta").mode("overwrite").saveAsTable(self.config.result_table)
        logger.info("Full comparison results stored in table: %s", self.config.result_table)
    except Exception as e:
        logger.error("Failed to save full comparison results: %s", e)
